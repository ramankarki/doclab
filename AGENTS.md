...
fetchUrl → turndown + GFM (HTML→MD) → chunkMarkdown (h2→h3→h4, fence-safe) → embed → SQLite + sqlite-vec
                                                                                        ↓
                                                                              hybridSearch (vector + keyword + RRF)
```
...
| `src/lib/search.ts` | Hybrid search: vector ANN + keyword + RRF fusion |
| `src/lib/embedder.ts` | Ollama/OpenAI/Voyage embedding abstraction |
| `src/db.ts` | SQLite schema, vec0 setup, CRUD operations |
...
bun run dist/cli.js   # Run local CLI (not global doclab)
```
