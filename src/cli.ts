...
async function cmdList() {
  ...
  const rows = sources.map((src) => {
    ...
    const isStale =
      intervalMs > 0 && src.fetchedAt && Date.now() - new Date(src.fetchedAt).getTime() > intervalMs
    return {
      name: src.name,
      url: src.url,
      version,
  ...
  for (const row of rows) {
    const urlTrunc = row.url.length > urlW ? row.url.slice(0, urlW - 1) + '…' : row.url
...
async function cmdSearch(args: string[]) {
  ...
  try {
    ...
    for (let i = 0; i < result.results.length; i++) {
      ...
      const fusionStr = r.fusionScore != null ? `fusion: ${r.fusionScore.toFixed(3)}` : ''
      ...
      const fetchedStr = r.fetchedAt ? `fetched ${r.fetchedAt.slice(0, 10)}` : ''
...
async function cmdMem() {
  ...
  if (existsSync(logsDir)) {
    ...
    if (logSize > 0) {
      console.log(
        `${c.label}Logs:${c.reset}   ${formatBytes(logSize)} ${c.dim}(${logsDir})${c.reset}`
      )
    }
  }
...
async function apiPost<T>(port: number, path: string, body: unknown): Promise<T | null> {
  try {
    const response = await fetch(`http://127.0.0.1:${port}${path}`, {
      method: 'POST',
...
`)
  ...
  console.log(
    `  ${c.cyan}list${c.reset}                      ${c.dim}List all sources with freshness${c.reset}`
  )
  console.log(
    `  ${c.cyan}pull${c.reset} ${c.dim}[name]${c.reset}               ${c.dim}Re-fetch sources${c.reset}`
  )
  console.log(
    `  ${c.cyan}search${c.reset} ${c.dim}<query>${c.reset} ${c.dim}[...]${c.reset}      ${c.dim}Hybrid search${c.reset}`
  )
  console.log(
    `  ${c.cyan}rebuild${c.reset}                   ${c.dim}Drop DB, re-index all from scratch${c.reset}`
  )
  console.log(
  ...
  console.log(
    `  ${c.cyan}--source${c.reset} ${c.dim}<name>${c.reset}           ${c.dim}Filter by source${c.reset}`
  )
  console.log(
  ...
  console.log(
    `  ${c.dim}$ ${c.reset}doclab search ${c.dim}"hooks" --source react-docs --kind article${c.reset}`
  )
