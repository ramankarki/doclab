  ...
  if (config.embedding.provider === 'ollama') {
    ...
    try {
      ...
      if (info.reachable) {
        ...
        log(
          `${c.warn}[WARN]${c.reset} Ollama unreachable. Install: brew install ollama && ollama pull ${info.model}`
        )
    ...
    try {
      ...
      if (info.reachable) {
        ...
        embeddingDims = info.dimensions
        log(
          `${c.success}[OK]${c.reset} ${config.embedding.provider}: connected (${info.model}, ${info.dimensions}d)`
        )
      } else {
        ollamaStatus = 'unreachable'
        log(
          `${c.warn}[WARN]${c.reset} ${config.embedding.provider}: API key not set. Embedding disabled.`
        )
      }
    } catch (e: any) {
      ollamaStatus = 'unreachable'
...
async function runOverdueRebuild(config: DlConfig) {
  ...
  const db = getDb()
  const sources = db
    .prepare('SELECT name, fetched_at FROM sources WHERE fetched_at IS NOT NULL')
    .all() as { name: string; fetched_at: string }[]

  if (sources.length === 0) return

  const now = Date.now()
  const overdue = sources.filter((s) => now - new Date(s.fetched_at).getTime() > intervalMs)

  if (overdue.length > 0) {
    const names = overdue.map((s) => s.name).join(', ')
    log(
      `${c.info}[doclab]${c.reset} Rebuild overdue for ${overdue.length} source(s): ${names}. Running pull...`
    )
...
function startRebuildTimer(config: DlConfig) {
  ...
  rebuildTimer = setInterval(async () => {
    if (!state) return
    log(
      `${c.info}[doclab]${c.reset} Auto-rebuild: checking ${state.config.sources.length} source(s)...`
    )
