...
export function loadConfig(): { config: DlConfig; errors: string[] } {
  ...
  if (!existsSync(CONFIG_PATH)) {
    ...
    return {
      config: empty,
      errors: [
        "No config yet. Add sources with: doclab add <url>",
      ],
    };
  }
  ...
  try {
    ...
    return {
      ...
      errors: [
        `dlconfig.json: Invalid JSON at line ${extractLineFromError(e)}.`,
      ],
    };
  }
  ...
  if (obj.sources !== undefined) {
    if (!Array.isArray(obj.sources)) {
      ...
      for (let i = 0; i < obj.sources.length; i++) {
        ...
        if (!isValidUrl(url)) {
          errors.push(
            `dlconfig.json: '${url}' is not a valid URL.`
          );
          continue;
        }
        ...
        if (seenNames.has(name)) {
          errors.push(
            `dlconfig.json: Duplicate source name '${name}'.`
          );
          continue;
        }
  ...
  if (obj.embedding !== undefined) {
    ...
    if (
      provider &&
      !["ollama", "openai", "voyage"].includes(provider)
    ) {
      errors.push(
  ...
  if (obj.rebuildInterval !== undefined) {
    ...
    if (!isValidInterval(ri)) {
      errors.push(
        `dlconfig.json: Invalid rebuildInterval '${ri}'. Using default: 24h.`
      );
    } else {
  ...
  if (obj.maxChunksPerQuery !== undefined) {
    ...
    if (isNaN(n) || n < 1) {
      errors.push(
        `dlconfig.json: Invalid maxChunksPerQuery. Using default: 5.`
      );
    } else {
  ...
  if (obj.idleTimeout !== undefined) {
    ...
    if (!isValidInterval(it)) {
      errors.push(
        `dlconfig.json: Invalid idleTimeout '${it}'. Using default: 30m.`
      );
    } else {
