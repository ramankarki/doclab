...
async function openaiEmbed(
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenAI embed failed: HTTP ${response.status} — ${errorBody.slice(0, 200)}`)
  }
...
async function voyageEmbed(
  ...
  if (!response.ok) {
    const errorBody = await response.text()
    throw new Error(`Voyage embed failed: HTTP ${response.status} — ${errorBody.slice(0, 200)}`)
  }
