...
export async function checkOllama(ollamaUrl: string = DEFAULT_OLLAMA_URL): Promise<{
  reachable: boolean
  models: OllamaModel[]
  error?: string
}> {
...
export async function ollamaEmbed(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Ollama embed failed: HTTP ${response.status} — ${errorBody.slice(0, 200)}`)
  }
