...
describe('chunker', () => {
  ...
  test('preserves code fences', () => {
    ...
    const tsChunk = chunks.find((c) => c.content.includes('```ts'))
  ...
  test('handles blog post with h2 headers', () => {
    ...
    const useStateChunk = chunks.find((c) => c.header.includes('useState'))
  ...
  test('breadcrumb paths are correct', () => {
    ...
    const corsChunk = chunks.find((c) => c.sectionPath.includes('CORS'))
    expect(corsChunk).toBeDefined()
